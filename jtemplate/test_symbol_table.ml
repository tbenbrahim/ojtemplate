...
struct
	...
	let test_suite = ("Symbol Table",[
			...
			("compound declaration a={}, a.b={}, a.b.c=1", fun () ->
						let s = SymbolTable.initialize() in
						SymbolTable.declare (Ast.Name("a")) (MapValue(Hashtbl.create 10)) s;
						SymbolTable.declare (Ast.CompoundName(["a";"b"])) (MapValue(Hashtbl.create 10)) s;
						SymbolTable.declare (Ast.CompoundName(["a";"b";"c"])) (IntegerValue(1)) s;
						true
			);
			...
			("a=1,a.b=2 should throw NotAMap(\"a\" \"a.b\")", fun () ->
						...
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(1)) s;
						try
							SymbolTable.declare (Ast.CompoundName(["a";"b"])) (IntegerValue(2)) s;
							false
						with
						| NotAMap ("a", "a.b") -> true
						| _ -> false
			...
			("reference to undeclared a.b.c when a={},a.b=1 should throw NotAMap(\"b\",\"a.b.c\") ", fun() ->
						let s = SymbolTable.initialize() in
						SymbolTable.declare (Ast.Name("a")) (MapValue(Hashtbl.create 10)) s;
						SymbolTable.declare (Ast.CompoundName(["a";"b"])) (IntegerValue(1)) s;
						try
							let _ = SymbolTable.get_value (Ast.CompoundName(["a";"b";"c"])) s in
							false
						with
			...
			("redeclaration of a: a=1, a=2 ", fun () ->
						let s = SymbolTable.initialize() in
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(1)) s;
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(2)) s;
			...
			("assigning key value to map in nested scope: a={},a.b=1 { { a.b=2 a.b=2?}}", fun () ->
						...
						let s = SymbolTable.push_scope s in
						let s = SymbolTable.push_scope s in
						SymbolTable.assign (Ast.CompoundName(["a";"b"])) (IntegerValue(2)) s;
			...
			("assignment of a in nested scope : a=1 { a=2 } a=2? ", fun () ->
						...
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(1)) s;
						let s = SymbolTable.push_scope s in
						SymbolTable.assign (Ast.Name("a")) (IntegerValue(2)) s;
						let s = SymbolTable.pop_scope s in
						SymbolTable.get_value (Ast.Name("a")) s = IntegerValue(2)
			);
			...
			("redeclaration to different type: a=1, a='1' ", fun() ->
						let s = SymbolTable.initialize() in
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(1)) s;
						SymbolTable.declare (Ast.Name("a")) (StringValue("1")) s;
						true
			);
			...
			("reassignment to different type: a=1, a='1' should fail with TypeMismatchInAssignment(\"a\",\"integer\",\"string\")", fun() ->
						...
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(1)) s;
						try
							SymbolTable.assign (Ast.Name("a")) (StringValue("1")) s;
							false
						with
			...
			("map redeclaration to different type: a={b:1}, a.b='1' ", fun() ->
						...
						SymbolTable.declare (Ast.Name("a")) (MapValue(Hashtbl.create 10)) s;
						SymbolTable.declare (Ast.CompoundName(["a";"b"])) (IntegerValue(1)) s;
						SymbolTable.declare (Ast.CompoundName(["a";"b"])) (StringValue("1")) s;
						true
			);
			...
			("assignment of a in nested scope visible in original table : a=1 { a=2 } a=2? ", fun () ->
						...
						SymbolTable.declare (Ast.Name("a")) (IntegerValue(1)) s1;
						let s2 = SymbolTable.push_scope s1 in
						SymbolTable.assign (Ast.Name("a")) (IntegerValue(2)) s2;
						SymbolTable.get_value (Ast.Name("a")) s1 = IntegerValue(2)
			);
