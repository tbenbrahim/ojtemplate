...
and runtime_variable_value =
	...
	| RFunctionValue of int * int * int * bool * runtime_statement list * (int, runtime_variable_value) Hashtbl.t option (* stack size, depth, argslen,varargs?, statements, closure vars *)
...
and runtime_statement =
	...
	| RProgram of runtime_statement list (* like a runtime_statement block but does not push scope*)
